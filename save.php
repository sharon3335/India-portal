    ...
    <table>
      ...
      <tr><td>Description:</td> <td><input type='text' class='form-control' id='address' /> </td> </tr>
	   <label>Select State</label>
                                    <select class="form-control" name="state" id="sname">
                     
                                     <option></option>
                                 <?php
								include('dbcon.php');
echo $sql="SELECT distinct sname FROM state";					
...
while($row=mysqli_fetch_array($result))
{
  ...
  <script>
    ...
    function initMap() {
      var location = { lat: 9.639, lng: 76.548 };
    ...
    function saveData() {
	  ...
	  var sname = escape(document.getElementById('sname').value);
    //  var type = document.getElementById('type').value;
      var latlng = marker.getPosition();
      var url = 'save_action.php?name=' + name + '&address=' + address +
	  '&sname=' + sname + '&lat=' + latlng.lat() + '&lng=' + latlng.lng();
